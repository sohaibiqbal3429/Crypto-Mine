...
const QUERY_TIMEOUT_MS = Number(process.env.AUTH_QUERY_TIMEOUT_MS || 3000)

export async function POST(request: NextRequest) {
  return withRequestTiming("api.auth.login", async () => {
    try {
      ...
      const user = await User.findOne(query).maxTimeMS(QUERY_TIMEOUT_MS)
      if (!user) {
      ...
      if (isTimeoutError(error)) {
        return NextResponse.json({ error: "Login request timed out" }, { status: 504 })
      }
